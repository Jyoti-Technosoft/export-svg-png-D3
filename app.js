...
function draw() {
  svg.data([graphModel]).call(diagram);
  updateSvgDownloadLink();
}
function updateSvgDownloadLink() {
  var rawSvg = new XMLSerializer().serializeToString(
    d3.select("#canvas svg").node()
  );
  var imgsrc = d3
    .select("#downloadSvgButton")
    .attr("href", "data:image/svg+xml;base64," + btoa(rawSvg));
